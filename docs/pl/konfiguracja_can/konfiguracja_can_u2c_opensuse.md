# Konfiguracja magistrali CAN na openSUSE z konwerterem U2C

> Dokument opisuje krok po kroku konfigurację magistrali CAN (Controller Area Network) w systemie openSUSE przy użyciu konwertera USB–CAN Lawicel U2C (lub kompatybilnego). Konfiguracja jest niezbędna do uruchomienia [`astro-mount-controller`](index.md) z rzeczywistymi napędami CANopen.
...
System openSUSE domyślnie używa:

- **[wicked](https://github.com/openSUSE/wicked)** — narzędzia do zarządzania interfejsami sieciowymi,
- **firewalld** — zapory sieciowej,
- **systemd** — systemu inicjalizacji i zarządzania usługami.

**Czym jest wicked?**

Wicked to niskopoziomowy framework do konfiguracji sieci, rozwijany pierwotnie przez SUSE (obecnie openSUSE). W odróżnieniu od `NetworkManager` (który jest warstwą użytkownika — GUI, `nmcli`) czy `systemd-networkd` (część systemd), wicked działa jako samodzielna usługa systemowa, która:

- zarządza interfejsami sieciowymi poprzez pliki w `/etc/sysconfig/network/ifcfg-*`
- wspiera zaawansowane funkcje: bonding, bridging, VLAN, TUN/TAP, interfejsy bezprzewodowe
- ma wbudowany system skryptów konfiguracyjnych w `/etc/wicked/scripts/` — wywoływanych podczas `ifup`/`ifdown`, dzięki czemu można konfigurować dowolny typ interfejsu, w tym **CAN**
- w przeciwieństwie do `NetworkManager` nie próbuje automatycznie wykrywać i zarządzać wszystkimi interfejsami — wymaga jawnej konfiguracji w `ifcfg-*`

> **Dlaczego wicked zamiast NetworkManager na openSUSE?**
> SUSE od zawsze stosuje model konfiguracji przez pliki `/etc/sysconfig/network/`, który jest prostszy do automatyzacji (skrypty, ansible) i bardziej przewidywalny niż automatyczne wykrywanie NetworkManagera. W openSUSE Leap jest to domyślny stos sieciowy od początku instalacji.

**Konsekwencje dla CAN:**

- Wicked **nie ma natywnej obsługi interfejsów CAN** (nie ma wbudowanego `Kind=can` jak `systemd-networkd`), dlatego do konfiguracji CAN potrzebny jest skrypt zewnętrzny (sekcja [5.1](#51-opcja-a-wicked-domyslna))
- Jeśli NetworkManager jest również zainstalowany (często na pulpitach GNOME/KDE), może przejąć zarządzanie `can0` — trzeba to wyłączyć przez `unmanaged-devices` (opisane w sekcji 5.1)
- Alternatywnie można całkowicie wyłączyć wicked i przejść na `systemd-networkd`, który ma wbudowaną obsługę CAN (sekcja [5.2](#52-opcja-b-systemd-networkd-alternatywna))

Poniższy przewodnik został przygotowany dla **openSUSE Leap 15.4+ / Tumbleweed**, ale ogólne zasady dotyczą również innych dystrybucji.
...
Moduł `gs_usb` jest wbudowany w standardowe jądro openSUSE. Załaduj go ręcznie:
...
openSUSE może korzystać z kilku metod zarządzania interfejsami. Poniżej trzy opcje — wybierz odpowiednią dla swojego środowiska.

### 5.1 Opcja A: wicked (domyślna)

```bash
# ── Utwórz skrypt konfiguracyjny CAN ──
sudo tee /etc/wicked/scripts/can-setup.sh << 'EOF'
#!/bin/bash
# Konfiguracja magistrali CAN z konwerterem U2C
...
# ── Włącz i uruchom wicked ──
sudo systemctl enable wicked
sudo systemctl restart wicked
```

> **Uwaga**: Jeśli używasz **NetworkManager** (np. w openSUSE Tumbleweed z Gnome/KDE), wyłącz go dla `can0`:
> ```bash
> sudo nmcli device set can0 managed no
> ```
> lub utwórz plik `/etc/NetworkManager/conf.d/90-can-unmanaged.conf`:
> ```
> [keyfile]
> unmanaged-devices=interface-name:can0
> ```

### 5.2 Opcja B: systemd-networkd (alternatywna)
...
### 5.3 Opcja C: niestandardowa usługa systemd

Jeśli wolisz mieć pełną kontrolę (np. z obsługą błędów i restartem):

> **Uwaga**: Usługa poniżej **załadowuje niezbędne moduły jądra** przed konfiguracją interfejsu — w przeciwieństwie do opcji A i B, które polegają na `modules-load.d`. Jest to konieczne, ponieważ `After=network.target` nie gwarantuje, że moduły CAN są już załadowane.
...
Type=oneshot
RemainAfterExit=yes
# Krok 1: Załaduj moduły jądra (jeśli nie są jeszcze załadowane)
# Krok 2: Poczekaj aż interfejs can0 się pojawi (do 5 sekund)
# Krok 3: Skonfiguruj prędkość i włącz
ExecStart=/bin/sh -c '\
    modprobe can && \
    modprobe can_raw && \
    modprobe can_dev && \
...
| 4 | Warunek: UP + bitrate=1000000 | Jeśli już skonfigurowany poprawnie — pomija (zapobiega konfliktowi z wicked/NetworkManager) |
...
| 7 | Fallback: `ip link set can0 up` | Jeśli zmiana bitrate się nie powiodła (np. U2C już zablokowany przez inny sterownik) — podnosi istniejący interfejs z domyślnym bitrate |

**Najczęstsza przyczyna błędu `Device or resource busy`:**

Interfejs `can0` został już skonfigurowany przez inny mechanizm (wicked, NetworkManager, udev lub poprzednie uruchomienie usługi) i jego parametry czasowe (bit timing) są już zablokowane przez sterownik `gs_usb`. Nowa usługa radzi sobie z tym przez:

1. Sprawdzenie czy interfejs jest już UP z poprawnym bitrate → jeśli tak, pomija konfigurację
2. Jeśli zmiana bitrate się nie powiodła → podnosi interfejs takim jaki jest (z domyślnym bitrate U2C ~500 kbit/s lub ostatnio skonfigurowanym)

Aby sprawdzić aktualną konfigurację:
```bash
ip -details link show can0
# Szukaj: bitrate XXXX, state UP/DOWN
```

Jeśli potrzebujesz konkretnego bitrate innego niż domyślny, wyłącz najpierw inne mechanizmy zarządzania (sekcje 5.1, 5.2) lub zresetuj U2C przez odłączenie i ponowne podłączenie kabla USB.

**Diagnostyka gdy usługa nie wystartuje:**

```bash
# Podgląd błędów
sudo journalctl -xeu can-setup.service

# Ręczne wykonanie kroków:
sudo modprobe can can_raw can_dev gs_usb
lsmod | grep can
# Sprawdź czy gs_usb jest załadowany

# Czy U2C jest widoczny w USB?
lsusb | grep -i "1b7d\|Lawicel\|CAN"

# Czy po załadowaniu modułów pojawił się can0?
ip link show can0

# Ręczne uruchomienie konfiguracji:
sudo ip link set can0 type can bitrate 1000000
sudo ip link set can0 up
```
...
Konfiguracja konwertera U2C na openSUSE sprowadza się do kilku kroków:
...
**Konfiguracja CAN na innych dystrybucjach:**

- [`konfiguracja_can_u2c_ubuntu_debian.md`](konfiguracja_can_u2c_ubuntu_debian.md) — Ubuntu/Debian,
- [`konfiguracja_can_u2c_fedora.md`](konfiguracja_can_u2c_fedora.md) — Fedora/RHEL/CentOS,
- [`konfiguracja_can_u2c_arch.md`](konfiguracja_can_u2c_arch.md) — Arch Linux/Manjaro.
