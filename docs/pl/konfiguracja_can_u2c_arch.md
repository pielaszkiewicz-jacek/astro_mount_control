# Konfiguracja magistrali CAN na Arch Linux z konwerterem U2C

> Dokument opisuje krok po kroku konfigurację magistrali CAN (Controller Area Network) w systemie **Arch Linux** (oraz pochodnych: Manjaro, EndeavourOS, Garuda) przy użyciu konwertera USB–CAN Lawicel U2C (lub kompatybilnego). Konfiguracja jest niezbędna do uruchomienia [`astro-mount-controller`](index.md) z rzeczywistymi napędami CANopen.
...
Arch Linux domyślnie używa:

- **systemd-networkd** — domyślnie dostępnego, ale **nie zawsze domyślnie włączonego** narzędzia do zarządzania interfejsami sieciowymi,
- **NetworkManager** — często instalowanego na desktopach (zwłaszcza w środowiskach GNOME, KDE),
- **systemd** — systemu inicjalizacji i zarządzania usługami,
- **mkinitcpio** — systemu initramfs (moduły jądra można ładować wcześniej, jeśli potrzebne przed zamontowaniem systemu plików).

**Cechy charakterystyczne Arch Linux:**

- **Model „rolling release"** — jądro i oprogramowanie są stale aktualizowane; moduły CAN (`gs_usb`) są zawsze najnowsze.
- **Brak domyślnego managera sieci** — podczas podstawowej instalacji Arch Linux nie ma ani NetworkManagera, ani wicked; `systemd-networkd` jest jedynym dostępnym narzędziem, ale wymaga ręcznego włączenia.
- **AUR (Arch User Repository)** — zawiera narzędzia takie jak `can-utils` (dostępne również w oficjalnych repozytoriach).
- **Systemd-networkd ma wbudowaną obsługę CAN** (`Kind=can`) — to najprostsza i zalecana metoda trwałej konfiguracji na Arch Linux.

Poniższy przewodnik został przygotowany dla **Arch Linux (aktualna wersja rolling)** oraz jego pochodnych (Manjaro, EndeavourOS, Garuda Linux).
...
sudo pacman -S --noconfirm       \
    wireshark-qt                 \   # analiza ramek CAN (GUI)
...
Moduł `gs_usb` jest wbudowany w standardowe jądro Arch Linux. Załaduj go ręcznie:
...
Aby moduły ładowały się automatycznie po restarcie, utwórz plik konfiguracyjny:
...
#### 3.2.1 Ładowanie modułów w initramfs (opcjonalne)

Jeśli potrzebujesz interfejsu CAN **przed zamontowaniem systemu plików** (np. na embedded Arch z rootem na NFS po CAN), dodaj moduły do mkinitcpio:

```bash
# Dodaj moduły do pliku konfiguracyjnego mkinitcpio
...
> To nie jest wymagane dla typowej konfiguracji desktopowej — moduły załadowane przez `modules-load.d` są dostępne zaraz po uruchomieniu jądra.
...
Arch Linux może korzystać z kilku metod zarządzania interfejsami. Poniżej trzy opcje — wybierz odpowiednią dla swojego środowiska.

### 5.1 Opcja A: systemd-networkd (zalecana)

**systemd-networkd** ma wbudowaną obsługę interfejsów CAN przez `Kind=can` — na Arch Linux jest to najprostsza i najbardziej niezawodna metoda.

> **Uwaga**: Na czystej instalacji Arch Linux `systemd-networkd` nie jest domyślnie włączony. Należy go włączyć ręcznie.
...
**Sprawdzenie, czy konfiguracja została zastosowana:**
...
#### 5.1.1 Wyłączenie can0 w NetworkManager (jeśli używasz NetworkManagera)

Jeśli na Arch Linux używasz NetworkManagera (częste na desktopach GNOME/KDE), zablokuj zarządzanie `can0`:
...
Jeśli nie chcesz używać systemd-networkd (np. używasz NetworkManagera i nie chcesz mieszać backendów), utwórz samodzielną usługę systemd:
...
### 5.3 Opcja C: udev + ip (dla zaawansowanych użytkowników)

Arch Linux jest znany z podejścia „zrób to sam". Można skonfigurować CAN bezpośrednio przez regułę udev, co jest szczególnie przydatne dla użytkowników, którzy nie chcą używać systemd-networkd ani osobnej usługi:

```bash
sudo tee /etc/udev/rules.d/99-can-setup.rules << 'EOF'
# Konfiguracja CAN po podłączeniu U2C
...
> **Zalety**: Bardzo proste, działa natychmiast po podłączeniu U2C, nie wymaga systemd-networkd.
> **Wady**: Działa tylko dla konkretnego bitrate (trzeba zmienić w regule dla innego), nie obsługuje zaawansowanej diagnostyki.
...
### 8.6 Konflikt z NetworkManager (Arch Linux z pulpitem GNOME/KDE)

**Objaw**: Interfejs `can0` znika po restarcie lub nie można go skonfigurować.

**Przyczyna**: NetworkManager przejmuje zarządzanie `can0` i blokuje konfigurację przez `ip`.

**Rozwiązanie**:

```bash
# Wyłącz zarządzanie can0 przez NetworkManager
sudo nmcli device set can0 managed no

# Lub utwórz plik konfiguracyjny:
sudo tee /etc/NetworkManager/conf.d/90-can-unmanaged.conf << 'EOF'
[keyfile]
unmanaged-devices=interface-name:can0
EOF
...
Konfiguracja konwertera U2C na Arch Linux sprowadza się do kilku kroków:
...
4. **Utwórz trwałą konfigurację** — przez systemd-networkd (zalecane), niestandardową usługę systemd lub regułę udev.
