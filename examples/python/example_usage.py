#!/usr/bin/env python3
"""
Astronomical Mount Controller - Python gRPC Client Example
...
class MountControllerClient:
    ...
    def stop(self):
        """Stop all motion."""
        self.stub.Stop(pb.Empty())
        print("Stopped all motion")
    
    def park(self):
        """Park the mount."""
        self.stub.Park(pb.Empty())
        print("Parking mount")
    
    def get_state(self):
        """Get the current controller state."""
        return self.stub.GetState(pb.Empty())
    ...
    def get_tpoint_parameters(self):
        ...
        return self.stub.GetTPointParameters(pb.Empty())
    
    def get_rotation_matrix(self):
        """Get the rotation matrix."""
        return self.stub.GetRotationMatrix(pb.Empty())
    ...
    def get_configuration(self):
        ...
        return self.stub.GetConfiguration(pb.Empty())
...
def print_state(state):
    ...
    if state.target:
        print(f"  Target: RA={state.target.ra:.3f}h, Dec={state.target.dec:.3f}°")
